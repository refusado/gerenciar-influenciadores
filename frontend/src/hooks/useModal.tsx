...
import { EditInfluencerForm } from '@/app/admin/influenciadores/components/form/edit-form';
import { RegisterInfluencerForm } from '@/app/admin/influenciadores/components/form/register-form';
    ...
    <ModalContext.Provider value={{ openModal, closeModal, resourceId }}>
      ...
      >
        <Dialog.Title className="mb-2 text-xl">
          Ver detalhes do influenciador #{resourceId}
        </Dialog.Title>
      </ModalContainer>
      ...
      >
        <Dialog.Title className="mb-2 text-xl">
          Editar influenciador #{resourceId}
        </Dialog.Title>
        <EditInfluencerForm influencerId={resourceId!} />
      </ModalContainer>
      ...
      >
        <Dialog.Title className="mb-2 text-xl">
          Associar influencer #{resourceId} uma marca
        </Dialog.Title>
      </ModalContainer>
...
function ModalContainer({
  ...
  return (
    <Dialog.Root open={open} onOpenChange={(isOpen) => !isOpen && closeModal()}>
      ...
      <Dialog.Content className="fixed left-[50%] top-[50%] z-50 max-h-[85vh] w-[90vw] max-w-[560px] -translate-x-1/2 -translate-y-1/2 overflow-y-auto bg-zinc-800 p-4">
        {children}
      </Dialog.Content>
    </Dialog.Root>
  );
